...
export async function fetchDailyPrices(symbol) {
  ...
  try {
    // TIME_SERIES_DAILY is available on the free tier
    // TIME_SERIES_DAILY_ADJUSTED requires a premium subscription
    const url = `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${symbol}&outputsize=compact&apikey=${apiKey}`;
    const res = await fetch(url, { next: { revalidate: 900 } });
    const json = await res.json();

    if (json["Note"] || json["Information"]) {
      // Rate limited or premium endpoint restriction
      console.warn(`Alpha Vantage rate limit hit for ${symbol}:`, json["Note"] || json["Information"]);
      return generateDemoData(symbol);
    }
    ...
    const prices = Object.entries(series)
      ...
      .map(([date, bar]) => ({
        date,
        close: parseFloat(bar["4. close"]), // free tier field (not "5. adjusted close")
